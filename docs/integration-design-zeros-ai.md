...
> ステータス: **方針確定（2026-06-18 ユーザー決定）**

> ## 🔒 確定方針（2026-06-18）── 本書の上位決定
> - **カタログ単一真実源 = 学習PF（本PF）**。ただし採用するカタログは**学習PFが既に保有するネイティブ・カタログ（`supabase/seed_*.sql` 系: L0.1/L0.7/L1/L2/L3/MT/横断テーマトラック）**であり、**ZEROS-AI 131本の移植は行わない**。源コンテンツは共通（コンテンツ工場）で、ZEROS-AI 固有の skill_deepdive / knowledge_deepdive / awareness 等で不足が出た場合は**学習PFでネイティブに作成**する。
> - したがって **`docs/migration/` のカタログ移植草案（catalog-schema-delta.sql / catalog-migration-plan.md）＝案B「ZEROS-AIの分類軸を courses に列追加移植」アプローチは取り下げ・撤去**（§5 のA/B/C比較は記録として残すが、結論は「学習PFネイティブ・カタログを正典」）。
> - **連携 = 診断 → 学習PFロードマップ割当**。ZEROS-AI は診断・処方エンジンに縮小し、診断結果（`band`＋`route`＋知識の穴）を学習PFの **ロードマップ（learning_path）割当として処方**する。段階的に MVP=band→レベル別ロードマップ、拡張=領域A〜Fの穴に応じた部分処方。突合キー=**email**。
> - ZEROS-AI の受講レイヤー（courses/materials/material_progress・受講ページ）は段階委譲・最終廃止（§7 Phase 5）。
...
|---|---|---|---|---|
| **0** | カタログ真実源の決定 | ✅ **確定済（2026-06-18）= 学習PFネイティブ・カタログを正典**（上部🔒方針） | 決定記録 | ― |
| **1** | 学習PFの前提整備（先行） | ①`/admin/*``/org/*` サーバー側ロールゲート追加 ②`getServiceRoleClient`/ユーザー作成ロジックの重複解消 ③admin/users に検索/ページネーション/編集・削除 | 修正PR | E2E(権限境界) |
| **2** | 認証・テナント基盤統合 | ロール写像実装・person↔users 突合（**email**）・organizations整合・`users.line_user_id` カラム新設(LINE突合する場合) | 突合バッチ＋API | E2E(ログイン/権限) |
| ~~**3**~~ | ~~コースカタログ統合（ZEROS-AI 131本移植）~~ → **取り下げ**。学習PFネイティブ・カタログを正典とし移植しない。不足領域は学習PFで作成 | ― | ❌ 着手しない |
| **4** | 連携IF構築（**診断→ロードマップ割当**） | 診断結果(`band`/`route`/穴)→学習PF `user_learning_paths` へロードマップ割当。MVP=band→レベル別ロードマップ／拡張=穴に応じた部分処方。＋進捗参照API＋サービス間認証 | 連携API | E2E(診断→割当) |
| **5** | ZEROS-AI受講機能の委譲 | ZEROS-AI courses/materials/material_progress・受講ページを段階停止・処方出力に縮小 | 縮小PR | リグレッション |
