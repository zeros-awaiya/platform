...
export default async function CoursesAdminPage() {
  ...
  try {
    ...
    // 2. Fetch courses with categories and ordered lessons
    const { data: courseData } = await supabase
      .from('courses')
      .select('*, categories(name), lessons(*)')
      .order('created_at', { ascending: false })
      
    // Sort lessons inside javascript to ensure they are ordered correctly
    courses = (courseData || []).map(course => ({
      ...course,
      lessons: (course.lessons || []).sort((a, b) => a.sort_order - b.sort_order)
    }))
